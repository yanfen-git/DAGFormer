...
def make_lgbm(scale_pos_weight=None, eval_metric="aucpr"):
    ...
    return LGBMClassifier(
        ...
        n_jobs=-1,
        objective="binary",
        metric=eval_metric,      # 'auc' 或 'aucpr'
        scale_pos_weight=scale_pos_weight,
        verbosity=-1,            # 静默（4.x 不再用 verbose 参数）
    )
...
def fit_with_early_stop(model, X_tr, y_tr, X_val, y_val, allow_early_stop=True):
    ...
    if not allow_early_stop:
        model.fit(
            ...
            callbacks=[log_evaluation(period=0)],   # 关闭训练日志
        )
        return "no_es"
...
def run_one_drug(base_dir: str, drug: str, out_dir: str, no_early_stop=False):
    ...
    model = make_lgbm(scale_pos_weight=spw, eval_metric="aucpr")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run LightGBM (Source->Target) on scAdaDrug datasets")
    ...
    parser.add_argument("--out_dir", type=str, default="./lgbm_results", help="输出目录")
    ...
    if results:
        ...
        out_csv = os.path.join(args.out_dir, f"lgbm_summary_{ts}.csv")
