...
def compute_spearman_correlation(features):
    ...
    Returns:
        numpy.ndarray: Spearman correlation matrix
    """
    corr_matrix, _ = spearmanr(features.T, axis=0)  # axis=0 for column-wise correlation
    return corr_matrix

def build_graph_using_spearman(features, percentile=20):
    """
    Build a graph based on Spearman correlation
    Args:
        features (numpy.ndarray): Feature matrix, shape (n_samples, n_features)
        percentile (float): Spearman correlation coefficient threshold, only nodes with correlation above this value will be connected
    Returns:
        dgl.DGLGraph: A graph constructed based on Spearman correlation coefficient
    """
    # Compute the Spearman correlation matrix
    corr_matrix = compute_spearman_correlation(features)

    # Get all off-diagonal correlation values
...
def load_data_drug(dataset, DRUG):
    ...
    # Build the graph using Spearman correlation
    adj, graph = build_graph_using_spearman(features, percentile=20)
